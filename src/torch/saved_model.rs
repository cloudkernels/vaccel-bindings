...
impl SavedModel {
    ...
    fn set_protobuf(&mut self, data: &[u8]) -> Result<()> {
        match unsafe {
            ffi::vaccel_torch_saved_model_set_model(self.inner, data.as_ptr(), data.len() as u64)
                as u32
        } {
            ffi::VACCEL_OK => Ok(()),
            err => Err(Error::Runtime(err)),
    ...
    fn set_checkpoint(&mut self, data: &[u8]) -> Result<()> {
        match unsafe {
            ffi::vaccel_torch_saved_model_set_checkpoint(self.inner, data.as_ptr(), data.len() as u64)
                as u32
        } {
            ffi::VACCEL_OK => Ok(()),
            err => Err(Error::Runtime(err)),
    ...
    fn set_var_index(&mut self, data: &[u8]) -> Result<()> {
        match unsafe {
            ffi::vaccel_torch_saved_model_set_var_index(self.inner, data.as_ptr(), data.len() as u64)
                as u32
        } {
            ffi::VACCEL_OK => Ok(()),
            err => Err(Error::Runtime(err)),
    ...
    pub fn get_checkpoint(&self) -> Option<&[u8]> {
        let mut size = Default::default();
        let ptr = unsafe { ffi::vaccel_torch_saved_model_get_checkpoint(self.inner, &mut size) };
    ...
    pub fn get_var_index(&self) -> Option<&[u8]> {
        let mut size = Default::default();
        let ptr = unsafe { ffi::vaccel_torch_saved_model_get_var_index(self.inner, &mut size) };

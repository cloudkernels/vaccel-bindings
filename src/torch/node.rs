...
pub struct Node {
    inner: *mut ffi::vaccel_torch_node,
}
...
impl Node {
    pub fn new(name: &str, id: i64) -> Self {
        ...
        let inner = unsafe { ffi::vaccel_torch_node_new(name, id) };
        assert!(!inner.is_null(), "Memory allocation failure");

        Node { inner }
    }

    pub unsafe fn from_vaccel_node(node: *mut ffi::vaccel_torch_node) -> Result<Self> {
        let name = ffi::vaccel_torch_node_get_name(node);
        if name.is_null() {
    ...
    pub fn id(&self) -> i64 {
        unsafe { ffi::vaccel_torch_node_get_id(self.inner) }
    }

    pub fn name(&self) -> String {
        let cmsg = unsafe { CStr::from_ptr(ffi::vaccel_torch_node_get_name(self.inner)) };
    ...
    pub(crate) fn inner(&self) -> *const ffi::vaccel_torch_node {
        self.inner
    }

    pub(crate) fn inner_mut(&mut self) -> *mut ffi::vaccel_torch_node {
        self.inner
    }
}
...
impl Drop for Node {
    fn drop(&mut self) {
        unsafe { ffi::vaccel_torch_node_destroy(self.inner) }
    }
}
...
impl From<&ffi::vaccel_torch_node> for Node {
    fn from(node: &ffi::vaccel_torch_node) -> Self {
...
impl From<&ffi::vaccel_torch_node> for TFNode {
    fn from(node: &ffi::vaccel_torch_node) -> Self {
        TFNode {
            name: unsafe {

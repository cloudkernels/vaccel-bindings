...
use crate::ffi::{
    vaccel_id_t, vaccel_resource, vaccel_torch_model, vaccel_torch_model_destroy, vaccel_torch_model_get_id,
    vaccel_torch_model_new, vaccel_torch_model_new_from_buffer,
};
...
impl FrozenModel {
    ...
    pub fn new(path: &Path) -> Result<Self> {
        let mut model = Box::new(vaccel_torch_model::default());
        ...
        match unsafe { vaccel_torch_model_new(&mut *model, c_str.as_ptr()) as u32 } {
    ...
    pub fn from_buffer(data: &[u8]) -> Result<Self> {
        ...
        match unsafe {
            vaccel_torch_model_new_from_buffer(&mut *model, data.as_ptr(), data.len() as u64) as u32
        } {
            VACCEL_OK => Ok(FrozenModel {
                inner: Box::into_raw(model),
            }),
    ...
    pub fn destroy(&mut self) -> Result<()> {
        match unsafe { vaccel_torch_model_destroy(self.inner) as u32 } {
            VACCEL_OK => Ok(()),
            err => Err(Error::Runtime(err)),
        }
    }
    ...
    pub fn inner() -> Option<*mut vaccel_torch_model>
}

impl VaccelResource for FrozenModel {
    fn id(&self) -> vaccel_id_t {
        unsafe { vaccel_torch_model_get_id(self.inner) }
    }
